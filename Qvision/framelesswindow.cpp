...
void FramelessWindow::setWindowIcon(const QString &image_path) {
    QImage* img=new QImage;
    if(!img->load(image_path)) //加载图像
    {
        return;
    }
    ui->icon->setPixmap(QPixmap::fromImage(*img).scaledToWidth(ui->icon->width()));
